...
// Мне кажетася, что отсчет сотнями смотрится внушительнее для "очков"
const gameScore = {
  score: null,
  scoreBox: null,
  scoreStart: 0,
...
scoreGrow(){
  this.score += this.scoreStart + 100;
  this.render();
},
drop() {
  this.score = 0;
...
const game = {
  ...
  reset() {
    ...
    this.stone.setCoordinates(this.getRandomFreeCoordinates());
    this.gameScore.drop();
    this.render();
  },
  ...
  getRandomFreeCoordinates() {
    const exclude = [this.food.getCoordinates(), this.stone.getCoordinates(),  ...this.snake.getBody()];
    // without ... -  [{}, [{}, {}, {}]] => with ... [{}, {}, {}, {}];
    while (true) {
      const rndPoint = {
  ...
  render() {
    this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.stone.getCoordinates());
  },
...
//рост змейки от поедаемой еды
  tickHandler() {
    ...
    if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
      ...
      this.food.setCoordinates(this.getRandomFreeCoordinates());
      this.stone.setCoordinates(this.getRandomFreeCoordinates());
      this.gameScore.scoreGrow();

      if (this.isGameWon()) this.finish();
    }
    if (this.stone.isOnPoint(this.snake.getNextStepHeadPoint())) {
      this.finish(); //остановка при втыкании в препятствие
    }
    this.snake.makeStep();
    this.render();
  },
